/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package latihan.pkg1;

/**
 *
 * @author inar
 * NAMA                 : Inar
 * KELAS                : TI-1A
 * NIM                  : A2.1900082
 * Deskripsi Program    : Perintah Menampilkan Value ke Layar
 */
public class Latihan1 {

    /**
     * @param args the command line arguments
     */
    public static void main(String[] args) {
        // TODO code application logic here
        System.out.println("BIODATA ROCKSTAR" + "\n" + "---------------------");
        System.out.println("NAMA\t\t: Inar");
        System.out.println("ALAMAT\t\t: Jalan Pasirkaliki no 01");
        System.out.println("TEMPAT LAHIR\t: Subang");
        System.out.println("TANGGAL LAHIR\t: 12 April 2000");
        System.out.println("---------------------");
        System.out.println("HOBI\t\t: Renang & Lari");
        System.out.println("MAKES\t\t: Sate");
        System.out.println("MIKES\t\t: Cendol");
    }
    
}